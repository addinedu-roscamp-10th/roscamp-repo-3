...
class FallInferenceStreamError(RuntimeError):
    """Raised when IF-PAT-005 subscribe or push processing fails."""


@dataclass(frozen=True)
class FallInferenceStreamConfig:
...
class FallInferenceStreamClient:
    def __init__(
        self,
        *,
        host=DEFAULT_HOST,
    ...
    def from_env(cls):
        return cls(
            ...
            last_seq=int(os.getenv("AI_FALL_STREAM_LAST_SEQ", "0") or "0"),
            connect_timeout_sec=float(
                os.getenv(
                    "AI_FALL_STREAM_CONNECT_TIMEOUT_SEC",
            ...
            reconnect_delay_sec=float(
                os.getenv(
                    "AI_FALL_STREAM_RECONNECT_DELAY_SEC",
    ...
    async def _open_connection(self):
        try:
            ...
            raise FallInferenceStreamError(
                f"AI fall inference stream connection failed: {exc}"
            ) from exc
    ...
    async def _send_subscribe_request(self, reader, writer):
        ...
        try:
            ...
            raise FallInferenceStreamError(
                f"IF-PAT-005 subscribe ack read failed: {exc}"
            ) from exc
    ...
    async def _consume_push_batches(self, reader, *, on_results=None, max_batches=None):
        ...
        while max_batches is None or batch_count < int(max_batches):
            try:
                ...
                raise FallInferenceStreamError(
                    f"IF-PAT-005 push frame read failed: {exc}"
                ) from exc
    ...
    def _parse_subscribe_ack(frame, *, sequence_no):
        if frame.message_code != MESSAGE_CODE_FALL_INFERENCE_RESULT_SUBSCRIBE:
            raise FallInferenceStreamError(
                f"unexpected IF-PAT-005 ack message_code: 0x{frame.message_code:04x}"
            )
        if frame.sequence_no != sequence_no:
            raise FallInferenceStreamError(
                f"unexpected IF-PAT-005 ack sequence_no: {frame.sequence_no}"
            )
        if not frame.is_response:
            raise FallInferenceStreamError("IF-PAT-005 subscribe ack must be a response frame.")
        ...
        if frame.is_error or result_code != "ACCEPTED":
            ...
            raise FallInferenceStreamError(
                f"IF-PAT-005 subscribe rejected: {result_code or 'ERROR'}"
    ...
    def _parse_push_batch(cls, frame):
        if frame.message_code != MESSAGE_CODE_FALL_INFERENCE_RESULT_SUBSCRIBE:
            raise FallInferenceStreamError(
                f"unexpected IF-PAT-005 push message_code: 0x{frame.message_code:04x}"
            )
        if not frame.is_push:
            raise FallInferenceStreamError("IF-PAT-005 result frame must be a push frame.")

        payload = frame.payload if isinstance(frame.payload, dict) else {}
        raw_results = payload.get("results")
        if not isinstance(raw_results, list):
            raise FallInferenceStreamError("IF-PAT-005 push payload.results must be a list.")
        ...
        if result_seq_values and batch_end_seq < max(result_seq_values):
            raise FallInferenceStreamError(
                "IF-PAT-005 batch_end_seq is smaller than a result_seq."
            )
    ...
    def _normalize_result(cls, item):
        if not isinstance(item, dict):
            raise FallInferenceStreamError("IF-PAT-005 result item must be an object.")
        ...
        normalized["frame_id"] = cls._required_text(
        ...
        if not isinstance(normalized.get("fall_detected"), bool):
            raise FallInferenceStreamError("IF-PAT-005 result.fall_detected must be bool.")
        return normalized
    ...
    def _required_text(cls, value, *, field_name):
        ...
        if normalized is None:
            raise FallInferenceStreamError(f"IF-PAT-005 result.{field_name} is required.")
        return normalized
    ...
    def _normalize_u32(value, *, field_name):
        try:
            ...
            raise FallInferenceStreamError(f"IF-PAT-005 {field_name} must be u32.") from exc
