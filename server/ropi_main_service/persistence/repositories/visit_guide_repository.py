from server.ropi_main_service.persistence.async_connection import async_execute, async_fetch_one
...
class VisitGuideRepository:
    ...
    async def async_find_patient(self, keyword: str):
        ...
        return self._format_patient(row)
    ...
    async def async_create_robot_guide_event(self, patient_name: str, room_no: str, member_id=None):
        description = f"[면회 안내] 대상={patient_name}, 목적지={room_no or '미지정'}, 안내 시작 요청"
        target_member_id = self._normalize_member_id(member_id)
