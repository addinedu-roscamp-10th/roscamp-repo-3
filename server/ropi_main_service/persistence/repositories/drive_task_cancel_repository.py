...
DRIVE_STATUS_CANCEL_REQUESTED = "CANCEL_REQUESTED"
REASON_USER_CANCEL_REQUESTED = "USER_CANCEL_REQUESTED"
CANCELLABLE_DRIVE_TASK_STATUSES = {
    "WAITING",
    "WAITING_DISPATCH",
    "READY",
    "ASSIGNED",
...
class DriveTaskCancelRepository:
    def get_drive_task_cancel_target(self, task_id):
        ...
        row = fetch_one(
            load_sql("drive/find_drive_task_for_cancel.sql"),
            (numeric_task_id,),
        )
        ...
        *,
        ...
        try:
            ...
            with conn.cursor() as cur:
                cur.execute(
                    load_sql("drive/lock_drive_task_for_cancel.sql"),
                    (numeric_task_id,),
                )
                row = cur.fetchone()
                response = self._record_drive_task_cancel_result(
                    cur,
                    row=row,
    ...
    async def async_record_drive_task_cancel_result(
        ...
        *,
        ...
        async with async_transaction() as cur:
            ...
            return await self._async_record_drive_task_cancel_result(
                cur,
                row=row,
                task_id=numeric_task_id,
                caregiver_id=caregiver_id,
    ...
    def _record_drive_task_cancel_result(
        self,
        cur,
        *,
        row,
        ...
        if plan["cancel_requested"]:
            ...
            cur.execute(
                load_sql("drive/update_drive_task_detail_cancel_requested.sql"),
        ...
        return self._build_drive_cancel_response(
            row=row,
            plan=plan,
            cancel_response=cancel_response,
        )

    async def _async_record_drive_task_cancel_result(
        self,
        cur,
        *,
        row,
    ...
    def _build_cancel_result_guard(self, row, *, task_id):
        if not row:
            return self._build_response(
                ...
                result_message="DRIVE task를 찾을 수 없습니다.",
        ...
        if not self._is_cancellable_status(row.get("task_status")):
            return self._build_response(
                result_code="REJECTED",
                result_message="이미 종료되었거나 취소할 수 없는 DRIVE task입니다.",
    ...
    def _build_cancel_write_plan(
        ...
        *,
        ...
        task_status = row.get("task_status")
        phase = row.get("phase")
        event_name = "DRIVE_TASK_CANCEL_REJECTED"
        severity = "WARNING"

        if cancel_requested:
            task_status = TASK_STATUS_CANCEL_REQUESTED
            phase = TASK_STATUS_CANCEL_REQUESTED
            event_name = "DRIVE_TASK_CANCEL_REQUESTED"
            severity = "INFO"
    ...
    def _normalize_cancel_result(cancel_response):
        ...
        result_code = str(cancel_response.get("result_code") or "UNKNOWN").strip()
        ...
        if result_message is None:
            result_message = (
                "DRIVE task 취소 요청이 접수되었습니다."
                if cancel_requested
                else "DRIVE task 취소 요청이 수락되지 않았습니다."
            )
    ...
    @classmethod
    def _build_drive_cancel_response(cls, *, row, plan, cancel_response):
        response = cls._build_response(
            ...
            phase=plan["phase"],
            assigned_robot_id=row.get("assigned_robot_id"),
            drive_status=DRIVE_STATUS_CANCEL_REQUESTED
            if plan["cancel_requested"]
            else row.get("drive_status"),
    ...
    def _is_cancellable_status(task_status):
        return str(task_status or "").strip() in CANCELLABLE_DRIVE_TASK_STATUSES
...
__all__ = ["DriveTaskCancelRepository"]
