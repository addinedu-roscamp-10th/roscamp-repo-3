...
_default_delivery_workflow_task_manager = None


class DeliveryWorkflowTaskManager:
    def __init__(self):
        self._tasks = set()
    ...
    async def join(self, *, timeout_sec: float | None = None) -> None:
        ...
        while self._tasks:
            ...
            if deadline is not None:
                ...
                if wait_timeout == 0.0:
                    raise TimeoutError("delivery workflow background tasks did not finish before timeout")
    ...
    async def shutdown(self, *, timeout_sec: float = 5.0, cancel_running: bool = True) -> None:
        ...
        try:
            ...
            logger.warning(
                "delivery workflow background task shutdown timed out",
                extra={"pending_count": self.pending_count},
            )


def get_default_delivery_workflow_task_manager() -> DeliveryWorkflowTaskManager:
    global _default_delivery_workflow_task_manager

    if _default_delivery_workflow_task_manager is None:
        _default_delivery_workflow_task_manager = DeliveryWorkflowTaskManager()

    return _default_delivery_workflow_task_manager
