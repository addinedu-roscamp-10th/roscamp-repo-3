...
class PatrolCancelService:
    ...
    def cancel_patrol_task(
        ...
        *,
        ...
        target_response = self.repository.get_patrol_task_cancel_target(task_id)
        ...
        return self.repository.record_patrol_task_cancel_result(
            task_id=task_id,
            caregiver_id=caregiver_id,
            reason=reason,
    ...
    async def async_cancel_patrol_task(
        ...
        *,
        ...
        async_cancel_target = getattr(
            self.repository,
            "async_get_patrol_task_cancel_target",
            None,
        )
        if async_cancel_target is not None:
            ...
            target_response = await asyncio.to_thread(
                self.repository.get_patrol_task_cancel_target,
                task_id,
            )
        if target_response.get("result_code") != self.ACCEPTED:
            return target_response
        ...
        async_record_cancel_result = getattr(
            self.repository,
            "async_record_patrol_task_cancel_result",
            None,
        )
        ...
        return await asyncio.to_thread(
            self.repository.record_patrol_task_cancel_result,
            task_id=task_id,
            caregiver_id=caregiver_id,
            reason=reason,
            cancel_response=cancel_response,
    ...
    def _validate_cancel_request(*, task_id, caregiver_id, reason):
        if not str(task_id or "").strip():
            return PatrolCancelService._invalid_request(
                "task_id가 필요합니다.",
                "TASK_ID_INVALID",
            )
    ...
    def _invalid_request(message: str, reason_code: str):
        response = DeliveryTaskCreateService._invalid_request(message, reason_code)
        response["task_type"] = "PATROL"
        return response

    @staticmethod
    def _rejected(message: str, reason_code: str):
        response = DeliveryTaskCreateService._rejected(message, reason_code)
        response["task_type"] = "PATROL"
        return response


__all__ = ["PatrolCancelService"]
