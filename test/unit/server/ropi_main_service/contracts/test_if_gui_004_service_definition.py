...
def test_if_gui_004_guide_command_service_exists_with_spec_fields():
    ...
    assert "int32 target_track_id" in sections[0]
    assert "string destination_id" in sections[0]
    assert "geometry_msgs/PoseStamped destination_pose" in sections[0]
    assert "wait_timeout_sec" not in sections[0]
    assert "finish_reason" not in sections[0]
