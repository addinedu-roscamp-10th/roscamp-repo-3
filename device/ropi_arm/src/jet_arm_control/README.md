...
| 파일 | 용도 |
| --- | --- |
| `jet_arm_control/arm1_node.py` | JetCobot arm action server |
| `launch/jet_arm.launch.py` | robot별 config를 읽어 노드 실행 |
...
source ~/roscamp-repo-3/device/ropi_arm/install/setup.bash
```
...
```bash
ros2 run jet_arm_control jet_arm_node --ros-args -p arm_id:=arm1 -p port:=/dev/ttyJETCOBOT
ros2 run jet_arm_control jet_arm_node --ros-args -p arm_id:=arm2 -p port:=/dev/ttyJETCOBOT
```
