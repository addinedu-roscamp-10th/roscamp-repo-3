...
| 파일 | 상태 |
| --- | --- |
| `ropi_delivery/mobile_controller_test.py` | 현재 관제 연동 성공 경로 |
| `ropi_delivery/transport_control_node.py` | 이전/실험용 코드, 정식 운용 경로로 확정하지 않음 |
...
```bash
ros2 run ropi_delivery mobile_controller_test --ros-args \
  -p robot_id:=pinky2 \
  -p action_name:=/ropi/control/pinky2/navigate_to_goal

ros2 run ropi_delivery pinky_navigation_action_server
```
