...
| 파일 | 용도 |
| --- | --- |
| `fallen_detection/fallen_detection_client_tcp.py` | 카메라 전송, TCP alarm 수신, 순찰 navigation 메인 노드 |
| `fallen_detection/fallen_alarm_buzzer.py` | `/fall_alarm`을 받아 부저 제어 |
| `fallen_detection/fallen_alarm.py` | `/fall_alarm` 기반 알람 처리 후보 코드 |
| `fallen_detection/camera.py` | Picamera2 기반 카메라 래퍼 |
| `fallen_detection/buzzer.py` | GPIO 부저 제어 |
| `fallen_detection/led.py`, `fallen_detection/pinkyled.py` | LED 제어 코드 |
...
```bash
ros2 run fallen_detection fallen_alarm_buzzer
```
...
- Nav2 map/parameter는 `ropi_pinky_config/config/pinky3`, `ropi_pinky_config/maps/pinky3`에서 관리한다.
